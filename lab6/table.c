 ...
 *
 * Description: takes a pivot point and places all greater values
    after it and all lower values before it
 *
 *
*/
int partition(int lower, int upper, int (*compare)(), void* elts[]) {
	void *temp;
	int x = lower;
	for (int i = lower; i < upper; i++) {
		if ((compare)(elts[i], elts[upper]) < 0) {
			temp = elts[x];
			elts[x] = elts[i];
			elts[i] = temp;
			x++;
 ...
 *
 * Description: Loops through the list and creates several pivot points to sort the list
 *
*/
void quickSort(int lower, int upper, int (*compare)(), void* elts[]) {
	int location;
	if (lower < upper) {
		location = partition(lower, upper, compare, elts);
		quickSort(lower, location-1, compare, elts);
		quickSort(location+1, upper, compare, elts);
	}
}
...
void *getElements(SET *sp)
{
	...
	quickSort(0, j-1, sp->compare, elts);
	return elts;
}
